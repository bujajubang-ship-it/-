...
empathy_points는 2-3개를 실제 댓글/카페 데이터에서 인용해 작성하세요. share_triggers의 공감 포인트도 이 실제 데이터와 연결하세요."""
    ...
    async def autofill_worksheet(self, keyword: str, ref_videos: List[Dict] = None,
        ...
        if knowledge_text:
            kt_section = f"""
== ★ 사장님 키 컨텐츠 작성 강의 (제목·도입부·본문 작성 시 반드시 이 작성법을 적용) ==
{knowledge_text}

[강의 적용 지침] 위 강의에서 가르친 키 컨텐츠 작성 원리(전환 멘트, 자격 입증, 디테일로 신뢰 형성, 도입부 스타일 등)를 titleCopy·introScript·bodyScript에 실제로 녹여내세요. 강의 개념을 이 주제에 맞게 적용하되 억지로 끼워넣지는 마세요.
"""
