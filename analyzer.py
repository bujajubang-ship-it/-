...
【워크시트 영상 기획 — "워크시트대로 기획해줘 / 이 주제로 영상 기획해줘" 요청 시 이 양식 전체로 완결된 기획을 낸다】
사용자가 주제·키워드를 주며 기획을 요청하면(예: "○○ 주제로 워크시트대로 기획해줘"), 아래 '기획 워크시트' 순서·형식 그대로 하나도 빠짐없이 작성한다. 대화에 레퍼런스 영상·댓글 데이터가 없으면 '지식탭 지식'과 채널 원칙으로 최대한 채우고, 근거 없이 추정한 부분은 "(추정)"으로 표기한다.

★ 반드시 '끝점(고객 언어)'부터 설계 — 이 영상 본 시청자가 뒤에서 지인에게 뭐라고 말할지(예: "공감돼서 지인한테 소개했잖아", "소탈·진정성 있어서 도와주고 싶어")를 먼저 정하고, 제목·도입부·본문을 그 반응이 나오도록 역산해 설계한다.
★ 경쟁영상을 베끼는 게 아니라 '디벨롭': (1)분해 — 잘된 도입부/제목을 beat(마디)로 쪼갬 → (2)기제 — 각 beat가 시청자 심리에 왜 먹히는지 한 문장 → (3)이식 — 뼈대(순서·심리기제)는 두고 소재만 내 주제로 갈아끼움.

[출력 순서·형식 — 마크다운 소제목으로 보기 좋게, 아래 11칸을 순서대로 다 채운다]
1. **기획 제목(주제)**
2. **끝점 (시청자 예상 대화)** — 시청자가 서로·지인에게 나눌 대화 3~4줄 (대화체)
3. **① 썸네일 분석** — 레퍼런스가 있으면 "-주제 / -클릭한 이유" 2줄 (없으면 생략)
4. **② 도입부 분석** — 참고 영상 도입부를 beat 단위 5~8줄, `[beat] 대사·장면 요약 → (왜 먹히는지 심리 기제)`
5. **공감 포인트** — 실제 유튜브 댓글·네이버 카페 원문 인용 2~4개, 끝에 "좋아요 N" 메타. 데이터 없으면 시청자 고민을 압축하고 "(추정)" 표기
6. **제목 추천 5개** — 각 '선택·후회·조건' 중 하나 이상을 담고, 4변형(①동일주제 ②쉬운단어 ③범위확장 ④강조수식어)을 섞어 다양하게. 각 줄 끝에 (선택/후회/조건) 표시
7. **섬네일 문구 3~4개** — 제목과 '다른 각도'로(제목+섬네일 한 세트), 핵심 5~12자, 결과·감정·공감 중심. 각 줄 끝에 (결과/감정/공감) 표시
8. **섬네일 디자인·촬영 묘사** — 바로 촬영 가능하게: 인물 표정·포즈·시선, 텍스트 위치·크기·색(어떤 문구를 어디에), 배경·소품, 색감·분위기, 구도
9. **도입부 원고(30초)** — 구어체·짧은 문장. ⓐ 섬네일/제목 문구를 30초 안에 '받아주기(callback)' ⓑ 섬네일 기대치를 '넘는' 임팩트. 문장은 지식탭 강의의 도입부 공식(공감[문제제기·유인·사례]→정보 등)을 실제로 진하게 적용
10. **본문 원고** — 구어체. 지식탭 강의의 본문 원리(주장·근거·원리·예시·반론·대안, 솔직함, 관점 넣기)를 적용하고, 위 공감 포인트에서 드러난 실제 욕구·불안을 녹인다
11. **참고 메모** — 참고한 영상·자료 (있으면 제목·URL)

작성 후 사용자가 특정 칸(제목만·도입부만 등)을 더 달라거나 고쳐달라 하면 그 칸만 재작성한다. 지식탭 강의 규칙은 원고에 실제로 녹이되 주제에 안 맞으면 억지로 끼워넣지 않는다.
...
{{
  ...
  "memo": "참고 영상: 제목 — URL"
    ...
    async def plan_jjachi(self, topic: str, answers: Dict = None,
        ...
        """'짜치는 기획' — 사장님이 워크시트 질문에 답한 '진짜 현실'을 재료로,
        AI는 지어내지 않고 조합·정리해 '짜치지만 공감가는' 기획안을 만든다."""
        answers = answers or {}
        QLABELS = [
            ("target", "이 영상 볼 사장님(타겟) — 어떤 식당·상황"),
            ("pain", "그 사장님이 이것 때문에 겪는 진짜 불편·고민 (운영자가 배달·상담하며 본·들은 것)"),
            ("innerVoice", "그 사장님이 속으로/뒤에서 하는 진짜 말 (실제 말투)"),
            ("howto", "이걸로 뭘 어떻게 해결·활용하나 (실제 방법·제품)"),
            ("honest", "솔직히 아쉬운 점 + 근데 왜 괜찮은지 (단점이자 장점)"),
            ("vs", "싼 것/경쟁사랑 진짜 차이 (솔직하게)"),
            ("mytake", "운영자가 팔면서/보면서 느낀 솔직한 생각·에피소드 (인간적인 한마디)"),
            ("endpoint", "영상 보고 시청자가 뭐라 말해줬으면 (끝점)"),
            ("filming", "촬영 가능 환경"),
        ]
        sheet = "\n".join(
            f"[{label}]\n{(answers.get(k) or '').strip()}"
            for k, label in QLABELS if (answers.get(k) or '').strip()
        )
        ...
        system_prompt = (
            "당신은 유튜브 기획 정리 코치입니다. 채널: 부자주방(주방용품점 사장이 운영, 외식업 사장 대상). "
            "★가장 중요한 역할★: 당신은 새 내용을 '지어내는' 사람이 아닙니다. "
            "사장님이 아래 워크시트에 적어준 '진짜 현실·생각'을 재료로, 그걸 '마음을 움직이는, 짜치지만 공감가는 기획안'으로 "
            "조합·정리·다듬는 사람입니다. 워크시트 답에 없는 사실·사례·디테일은 절대 지어내지 마세요(=조작). "
            "빈 답은 비워두고, 있는 재료만 살리세요. "
            "화자는 요리 안 하는 주방용품점 사장(수백 주방을 본 조력자) — 식당 1인칭 경험 지어내기 금지. "
            "톤: 폴리시된 판매자 말고 따뜻하고 솔직한 진짜 사람. 공감(시청자가 '딱 내 얘기' 느끼게) + 인간미. "
            "약점은 '단점이자 장점'으로만, '사지 마세요'식 자폭 금지(솔직함이 오히려 사고 싶게). "
            f"{CHANNEL_GOALS} "
            "반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이 순수 JSON만."
        )

        user_text = f"""영상 주제/제품: "{topic}"

== ★ 사장님이 워크시트에 적은 진짜 현실·생각 (이 재료로만 조합) ★ ==
{sheet or '(작성된 답 없음)'}
{market}{kt}

[작업 지침]
- 위 워크시트 답을 **재료로 조합·정리**해 기획안을 만드세요. 답에 없는 사실·사례·디테일은 지어내지 말 것. 빈 항목은 억지로 채우지 말 것.
- ★공감★: 도입은 워크시트의 '불편·고민'과 '진짜 속말'을 살려, 시청자가 "어 딱 내 얘기네" 하게 시작.
- ★인간미★: 폴리시된 판매자 말고 따뜻하고 솔직한 진짜 사람 톤. 옆에서 편하게 알려주듯. 화자는 요리 안 하는 주방용품점 사장(조력자).
- ★전략적 솔직함★: '단점이자 장점'(honest 답)을 인간적으로 녹이되, "사지 마세요/경쟁사 사세요"식 자폭 금지 — 솔직함이 오히려 사고 싶게.
- 본문은 워크시트 '실제 방법(howto)'을 유용하게 풀되, 그 사실 범위 안에서만. 이번 주제에만 충실.
- 끝점(viewerTalk)은 워크시트 'endpoint' 답을 반영.

아래 JSON으로 작성하세요:
{{
  "titles": ["제목 후보 3~5개 (유용하면서 공감·인간미 있는, 이번 주제로)"],
  "thumbnail": "섬네일 문구 + 구도 한 줄 (이번 주제)",
  "intro": "도입 대본 — 시청자 공감으로 시작하는 실제 구어체 대사 (그대로 읽으면 되게, 문장 끊어서)",
  "body": [{{"part":"본문 파트 소제목","script":"그 파트 원고 — 유용한 정보를 따뜻·솔직한 톤으로 (실제 말하듯 구어체)"}}],
  "outro": "마무리 대본 — 인간미 있게 편하게 (실제 대사, 부담 없는 소프트 마무리)",
