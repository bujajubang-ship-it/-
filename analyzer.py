...
empathy_points는 2-3개를 실제 댓글/카페 데이터에서 인용해 작성하세요. share_triggers의 공감 포인트도 이 실제 데이터와 연결하세요."""
    ...
    async def autofill_worksheet(self, keyword: str, ref_videos: List[Dict] = None,
        ...
        if knowledge:
            blocks = []
            for k in knowledge:
                s = (k.get("summary") or "").strip() or (k.get("content") or "")[:1500]
                if s:
                    blocks.append(f"[{k.get('title','강의')}]\n{s}")
            if blocks:
                knowledge_text = "\n".join(blocks)
...
{{
  ...
  "memo": "참고 영상: 제목 — URL"
    ...
    async def plan_jjachi(self, topic: str, viewer_heart: str = "", moving_script: str = "", my_story: str = "",
                          videos: List[Dict] = None, naver: List[Dict] = None,
                          knowledge: List[Dict] = None) -> Dict:
        """'짜치는 기획' — 감명 깊게 본 영상 스크립트가 '왜 마음을 움직였는지' 분해하고,
        강의 지식·실제 공감 데이터와 섞어 '사람의 마음을 얻는' 영상 기획을 만든다."""
        market = ""
        if videos:
        ...
        heart = f"\n== ★ 사장님이 원하는 '시청자의 마음' (이 영상 본 뒤 시청자가 이런 마음이 들었으면) ★ ==\n{viewer_heart.strip()}\n" if viewer_heart.strip() else ""
        moving = f"\n== 사장님이 감명 깊게 본 영상 스크립트 (선택 참고) ==\n{moving_script.strip()}\n" if moving_script.strip() else ""
        story = f"\n== 사장님의 진짜 이야기·경험(진정성 재료) ==\n{my_story.strip()}\n" if my_story.strip() else ""

        system_prompt = (
            "당신은 '사람의 마음을 얻는 콘텐츠'를 설계하는 유튜브 기획 코치입니다. "
            "채널: 부자주방 — 외식업 사장·자영업·창업 대중 대상. "
            "기술적 구조(훅·제목·구성)도 알지만, 이번 기획의 목적은 '정보 전달'이 아니라 "
            "'시청자가 이 사람을 응원하고 싶어지고, 마음이 움직이는 것'입니다. "
            "핵심 원리: 진정성(못난이 정서로 나를 낮춤)·솔직함·관점(팬은 관점에서 나온다)·"
            "실제 공감(지어내지 말고 실제 댓글/경험에서). "
            f"{CHANNEL_GOALS} "
            "반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이 순수 JSON만."
        )

        user_text = f"""영상 주제/핵심 내용: "{topic}"
{heart}{moving}{story}{market}{kt}

[작업 지침]
- 위 '사장님 원고·바이럴 강의(원초적 바이럴)'의 인사이트를 이 기획의 '작성 공식'으로 삼으세요. 기술적 구조는 유지하되, 거기에 '사람의 마음을 움직이는' 층을 얹는 겁니다.
0) ★가장 먼저★ '끝점(고객 언어)'을 설계하세요 — 이 영상 본 시청자가 '뒤에서 자기들끼리·지인에게' 뭐라고 말할지 구체적 대화체로 예상. 사장님이 '시청자의 마음'을 적어줬으면 그게 나오도록 설계. (예: "공감가서 지인한테 소개했잖아", "소탈·진정성 있어서 도와주고 싶더라", "배송 늦어도 저 사장님 열심히 하니까 사주고 싶어") 이 반응이 나오게 나머지 전부를 역산.
1) 이 정보성 주제에 '마음을 움직이는 공감 장면'을 구체적으로 설계해 넣으세요 — 실제 촬영/연출 가능한 장면으로. (짜치는=날것, 못난이 정서, 사장님 실패담·솔직함 활용)
2) 공감은 절대 지어내지 말고 실제 인기댓글·카페·사장님 이야기에서 인용하세요.
3) 목적은 정보 전달을 넘어 '이 사람 응원하고 싶다'는 감정을 만드는 것. 판매·정보는 그 감정 위에 얹기.

아래 JSON으로 작성하세요:
{{
  "viewerTalk": ["★끝점★ 이 영상 본 시청자가 뒤에서 서로/지인에게 나눌 예상 대화 3~4개 (실제 말투 대화체). 이게 기획의 목표점."],
  "coreEmotion": "이 영상이 시청자 마음속에서 건드리는 진짜 감정 한 문장",
  "empathyScenes": [{{"scene":"마음을 움직이는 공감 장면(실제 촬영·연출 가능하게 구체적으로)","why":"이 장면이 왜 마음을 움직이는지","place":"영상 어디에 넣을지(도입/전개/절정 등)"}}],
  "empathyPoints": [{{"quote":"실제 댓글/카페/사장님 경험 원문","source":"출처","insight":"이 안에 담긴 진짜 마음"}}],
  "authenticity": "사장님이 이 영상에서 드러낼 진정성 포인트 — 어떤 솔직함·실패담·약점을 보여줘야 마음이 열리는지 (못난이 정서 활용)",
  "emotionJourney": "도입→전개→절정→마무리까지 시청자 감정이 어떻게 흐르게 설계할지 (감정 여정)",
  "fanMoment": "정보를 넘어 '이 사람을 응원하고 싶다/구독하고 싶다'로 넘어가게 하는 결정적 한 방",
  "titles": ["감정·진정성 기반 제목 3~5개",
             "(기술적 후킹보다 마음을 건드리는 쪽)"],
  "introScript": "마음을 여는 도입부 대본 (구어체, 진정성·공감 먼저, 문장 끊어서)",
  "outroScript": "마음을 얻는 마무리 대본 (응원·연결·여운, 구어체)",
  "cta": "판매 전에 '사람으로 연결'되는 관계형 마무리 멘트",
  "checklist": ["이 기획이 진짜 마음을 얻는지 자가점검 3~4개"]
}}"""
