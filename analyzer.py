...
5) ★★가장 중요★★: 결과물은 '분석 나열'이 아니라 **이대로 들고 가서 찍으면 되는 촬영 대본(콘티)**이어야 한다. 각 장면마다 [어디서 / 누가 나오고 / 카메라가 뭘 잡고 / 실제 대사(그대로 읽으면 되게) / 어떤 느낌·연출로 / 자막]까지 구체적으로. 도입(훅·거울)→공감→조력자 등장(진정성)→진짜 원인→해결·현장 시연→감정 절정→끝점 유도 마무리 순서로 흐르게. 마지막 장면을 보면 위 '끝점(viewerTalk)' 반응이 자연스럽게 나오게 설계.
6) ★이번 주제("{topic}")에만 충실하게★ — 다른 주제(예시로 나온 냉장고 위칸 등)를 절대 끌어오지 말 것. 아래 예시 문구는 형식 참고용일 뿐, 반드시 이번 주제 상황으로 새로 쓸 것.

아래 JSON으로 작성하세요 (scenes가 핵심 — 순서대로 촬영 가능한 대본):
{{
  ...
  "thumbnail": "섬네일 문구 + 구도 한 줄 (이번 주제, 짜치지만 후킹되게)",
