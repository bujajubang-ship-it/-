...
{{
  ...
  "memo": "참고 영상: 제목 — URL"
    ...
    async def plan_jjachi(self, topic: str, moving_script: str = "", my_story: str = "",
        ...
        moving = f"\n== ★ 사장님이 '감명 깊게 본 영상' 스크립트 (마음을 움직인 레퍼런스) ★ ==\n{moving_script.strip()}\n" if moving_script.strip() else ""
...
{moving}{story}{market}{kt}

[작업 지침]
0) ★가장 먼저★ '끝점(고객 언어)'을 설계하세요 — 이 영상을 다 본 시청자가 '뒤에서 자기들끼리·지인에게' 뭐라고 말할지를 구체적 대화체로 예상. (예: "와 부자주방 영상 진짜 공감가서 지인한테 소개했잖아", "있어보이기보다 소탈하고 진정성 있어서 도와주고 싶더라", "배송 하루 늦어도 저 사장님 열심히 하니까 사주고 싶어") 그리고 '이 반응이 나오도록' 나머지 전부를 역산해서 설계하세요.
1) 감명 영상 스크립트가 '왜' 마음을 움직였는지 감정 기제로 분해하고, 그 원리를 이 주제에 이식하세요. (베끼는 게 아니라 '마음을 움직인 방식'을 가져오기)
2) 공감은 절대 지어내지 말고 실제 인기댓글·카페·사장님 이야기에서 인용하세요.
3) 강의 원리(진정성·못난이 정서·관점·솔직함·짜치는=날것·전염)를 도입부·마무리에 실제로 녹이세요.
4) 목적은 판매·정보가 아니라 '이 사람 응원하고 싶다'는 감정을 만드는 것. 판매는 그 다음.

아래 JSON으로 작성하세요:
{{
  "viewerTalk": ["★끝점★ 이 영상 본 시청자가 뒤에서 서로/지인에게 나눌 예상 대화 3~4개 (실제 말투 대화체). 이게 기획의 목표점."],
  "coreEmotion": "이 영상이 시청자 마음속에서 건드리는 진짜 감정 한 문장 (예: '나만 뒤처진 것 같은 불안', '아무도 안 알아주는 억울함')",
  "movingAnalysis": "감명 영상이 왜 마음을 움직였는지 3~5개 포인트로 분해 ('- [감정기제] 어떤 장면/대사 → 왜 마음이 움직이나' 형식). 스크립트 없으면 '(감명 영상 미제공)'",
  "empathyPoints": [{{"quote":"실제 댓글/카페/사장님 경험 원문","source":"출처","insight":"이 안에 담긴 진짜 마음"}}],
  "authenticity": "사장님이 이 영상에서 드러낼 진정성 포인트 — 어떤 솔직함·실패담·약점·관점을 보여줘야 마음이 열리는지 (못난이 정서 활용)",
  "emotionJourney": "도입→전개→절정→마무리까지 시청자 감정이 어떻게 흐르게 설계할지 (감정 여정)",
  "fanMoment": "정보를 넘어 '이 사람을 응원하고 싶다/구독하고 싶다'로 넘어가게 하는 결정적 한 방",
  "titles": ["감정·진정성 기반 제목 3~5개 (기술적 후킹보다 마음을 건드리는 쪽)"],
  "introScript": "마음을 여는 도입부 대본 (구어체, 진정성·공감 먼저, 문장 끊어서)",
  "outroScript": "마음을 얻는 마무리 대본 (응원·연결·여운, 구어체)",
  "cta": "판매 전에 '사람으로 연결'되는 관계형 마무리 멘트",
  "checklist": ["이 기획이 진짜 마음을 얻는지 자가점검 3~4개 (예: '시청자가 이 사람 응원하고 싶어질까?')"]
}}"""
