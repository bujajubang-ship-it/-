...
[역할 구분 — 매우 중요]
- 레퍼런스 영상 = "어떤 훅·구조가 먹혔는지"를 '참고'하는 용도 (소재·각도 힌트)
- 위 강의 = 도입부·본문의 '실제 문장을 어떻게 쓰는지' 정하는 '작성 공식'
→ introScript·bodyScript의 문장은 레퍼런스를 베끼는 게 아니라, **위 강의 원리로 직접 써야** 합니다. 특히 도입부는 강의의 도입부 공식(예: 공감[문제제기·유인·사례] → 정보 순서, '못난이 정서'로 나를 낮춰 공감, 유인에 이유 붙이기, 감각 공유 등)을 뼈대로 삼으세요.
[체크] introScript를 다 쓴 뒤, 위 강의 원리 중 최소 1~2개가 실제로 문장에 드러나는지 스스로 점검하고, 안 드러났으면 다시 쓰세요. (단, 주제에 안 맞는 원리를 억지로 넣지는 말 것)
"""
