...
empathy_points는 2-3개를 실제 댓글/카페 데이터에서 인용해 작성하세요. share_triggers의 공감 포인트도 이 실제 데이터와 연결하세요."""
    ...
    async def autofill_worksheet(self, keyword: str, videos: List[Dict] = None,
                                 naver: List[Dict] = None, transcripts: List[Dict] = None,
                                 viewtrap_refs: Optional[Dict] = None) -> Dict:
        """키워드 → 경쟁영상·댓글·카페·스크립트·ViewTrap을 분석해 기획 워크시트 칸을 자동 작성.
        반환 키는 프론트 WS_COLS와 1:1: name/keyword/thumbA/introA/empathy/titleCopy/introScript/bodyScript/memo."""
        market = ""
        if videos:
            market += f"\n== 유튜브 경쟁영상(조회수순)·인기댓글 ==\n{self._build_videos_text(videos, max_videos=6)}\n"
        if naver:
            market += f"\n== 네이버 카페 반응 ==\n{self._build_naver_text(naver)}\n"
        viewtrap_text = self._build_viewtrap_text(viewtrap_refs)
        if viewtrap_text:
        ...
        if transcripts:
            ...
            for t in transcripts:
                if t.get("text"):
                    src = "자동자막" if t.get("method") == "caption" else "받아쓰기"
                    blocks.append(f"[{t.get('title','경쟁영상')} · {src}]\n{t['text'][:4000]}")
            if blocks:
                script_section = "\n== 경쟁영상 실제 스크립트(도입부·본문 구조 분석용) ==\n" + "\n---\n".join(blocks) + "\n"
...
- thumbA(① 썸네일 분석): 가장 잘된 경쟁영상 썸네일 기준. 정확히 두 줄.
    "-주제 : (썸네일이 다루는 핵심 주제)"
    "-클릭한 이유 : (시청자의 어떤 고민/궁금증을 해결해줘서 클릭하는지)"
- introA(② 도입부 분석): 경쟁영상 실제 스크립트의 앞 30초 구조를 분해. '- '로 시작하는 5~8줄.
    기존상식 뒤집기→기대감 형성→공감→문제제시→전문성 연결 같은 흐름을 짚고, [공감] 표시도 활용. 스크립트가 없으면 "(스크립트 없음 — 추정)" 명시.
- empathy(공감 포인트): 실제 유튜브 인기댓글/네이버 카페 원문을 인용. 각 항목 끝에 "좋아요 N" 메타 표기. 2~4개. 데이터 없으면 "추정" 표기.
- titleCopy(④ 제목·썸네일 문구): 4가지 방법을 각각 적용해 4줄. ViewTrap 성과영상·핫비디오의 검증된 제목 언어패턴이 있으면 그 패턴을 이 주제로 카피·변형해 반영.
    "①동일주제 활용 : ___" / "②쉬운단어(동의어) : ___" / "③단어 범위 확장 : ___" / "④강조 수식어 : ___"
- introScript(⑥ 도입부 원고): 잘된 경쟁 도입부 틀을 내 주제로 변형. 30초 분량, 구어체, 문장을 끊어서 줄바꿈. 공감 → 기대감 → 문제제시 흐름.
- bodyScript(⑦ 본문 원고): 문제→이유→해결→이득 순서, 구어체. 댓글에서 드러난 시청자 욕구를 반영.
